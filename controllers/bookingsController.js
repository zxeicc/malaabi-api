...
exports.updateBookingStatus = async (req, res) => {
  try {
    ...
    if (rows.length > 0) {
      ...
      const subject = isConfirmed ? "✅ تم قبول حجزك في ملاعبي" : "❌ تم رفض حجزك في ملاعبي"
      ...
      const html = isConfirmed ? `
        <div style="font-family:Arial,sans-serif;direction:rtl;max-width:500px;margin:auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
          <div style="background:linear-gradient(135deg,#16a34a,#14532d);padding:24px;text-align:center">
            <h1 style="color:white;margin:0">⚽ ملاعبي</h1>
          </div>
          <div style="padding:24px">
            <h2 style="color:#16a34a">✅ تم قبول حجزك!</h2>
            <p>مرحباً <strong>${b.player_name}</strong>، تم قبول حجزك بنجاح.</p>
            <div style="background:#f3f4f6;border-radius:8px;padding:16px;margin:16px 0">
              <p>🏟️ <strong>الملعب:</strong> ${b.field_name}</p>
              <p>📍 <strong>الموقع:</strong> ${b.location}</p>
              <p>📅 <strong>التاريخ:</strong> ${date}</p>
              <p>⏰ <strong>الوقت:</strong> ${b.time_slot}</p>
              <p>⏱ <strong>المدة:</strong> ${b.duration} ساعة</p>
              <p style="color:#16a34a;font-size:18px"><strong>💰 الإجمالي: ${b.total} ر.س</strong></p>
            </div>
        ...
        <div style="font-family:Arial,sans-serif;direction:rtl;max-width:500px;margin:auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
          ...
          <div style="padding:24px">
            ...
            <p>مرحباً <strong>${b.player_name}</strong>،</p>
            <p>نأسف، حجزك في <strong>${b.field_name}</strong> بتاريخ <strong>${date}</strong> الساعة <strong>${b.time_slot}</strong> تم رفضه.</p>
            <p>يمكنك اختيار وقت آخر والمحاولة مجدداً.</p>
            <p style="color:#6b7280;font-size:13px">شكراً لاستخدامك منصة ملاعبي ⚽</p>
          </div>
        </div>`
      ...
      .then(() => console.log("✅ Email sent to:", b.player_email))
